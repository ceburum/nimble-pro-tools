        ...
        <DialogContent className="sm:max-w-xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          {step === 'services' && (
            <div className="flex flex-col flex-1 min-h-0">
              ...
              {/* Selected Services Summary - Fixed at bottom */}
              {selectedServices.length > 0 && (
                <div className="border-t pt-4 mt-2 space-y-3 flex-shrink-0">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {selectedServices.reduce((s, i) => s + i.quantity, 0)} items
                    </span>
                  </div>
                  <ScrollArea className="max-h-32">
                    <div className="space-y-2 pr-2">
                      {selectedServices.map(({ service, quantity }) => (
                        <div key={service.id} className="flex items-center justify-between text-sm bg-muted/30 p-2 rounded-lg">
          ...
          {step === 'schedule' && (
            <ScrollArea className="flex-1 min-h-0 pr-4">
              <div className="space-y-4 pb-2">
                ...
                {/* Notes - now uses Textarea for better multiline support */}
                <div className="space-y-2">
                  <Label>Appointment Notes</Label>
                  <Textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder="Special requests, client preferences, or any notes for this appointment..."
                    rows={3}
                    className="resize-none"
                  />
                  <p className="text-xs text-muted-foreground">
                    Notes will be saved with the appointment and visible on the invoice.
                  </p>
                </div>
          ...
          {/* Action Buttons - Fixed at bottom for schedule step */}
          {step === 'schedule' && (
