...
export function MileageCard() {
  ...
  const weeklyMiles = mileageEntries.filter(entry => new Date(entry.date) >= weekStart).reduce((sum, entry) => sum + entry.miles, 0);

  // Calculate expected odometer
  const milesTrackedSinceCalibration = calibration ? mileageEntries.filter(entry => new Date(entry.createdAt) >= new Date(calibration.date)).reduce((sum, entry) => sum + entry.miles, 0) : 0;
  ...
  const handlePositionError = (error: GeolocationPositionError) => {
    ...
    toast({
      ...
      description: 'You can enter distance manually when done'
    });
  };
  const startTracking = () => {
  ...
  return <>
      <div className="rounded-xl border p-4 shadow-sm border-muted bg-secondary">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              {isTracking ? <>
                  ...
                  <p className="text-xl font-bold text-card-foreground">
                    {weeklyMiles.toLocaleString('en-US', {
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Odometer info */}
            {expectedOdometer && !isTracking && <div className="text-right hidden sm:block">
                <p className="text-xs text-muted-foreground">Est. Odometer</p>
                <p className="text-sm font-medium text-card-foreground">
                  {expectedOdometer.toLocaleString('en-US', {
            ...
            <Popover>
              ...
              <PopoverContent className="w-72" align="end">
                <div className="space-y-3">
                  ...
                  {calibration && <div className="p-2 bg-muted/50 rounded text-xs space-y-1">
                      ...
                      <div className="flex justify-between font-medium border-t pt-1 mt-1">
                        ...
                        <span>{expectedOdometer?.toLocaleString('en-US', {
                        maximumFractionDigits: 0
                      })} mi</span>
                      </div>
                    </div>}
                  <div className="flex gap-2">
                    <Input type="number" placeholder="Current odometer" value={odometerInput} onChange={e => setOdometerInput(e.target.value)} className="flex-1" />
            ...
            {/* Main action button */}
            {isTracking ? <Button onClick={() => setIsTrackingDialogOpen(true)} size="sm" variant="destructive" className="gap-1">
                <Square className="h-4 w-4" />
                Stop
              </Button> : <Button onClick={() => setIsTrackingDialogOpen(true)} size="sm" className="gap-1">
      ...
      <Dialog open={isTrackingDialogOpen} onOpenChange={setIsTrackingDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="space-y-4">
            ...
            {isTracking ? <div className="space-y-4">
                ...
                {coordinates.length < 2 && <div className="space-y-2">
            ...
            {/* Recent trips */}
            {mileageEntries.length > 0 && !isTracking && <div className="space-y-2">
                <Label>Recent Trips</Label>
                <div className="max-h-32 overflow-y-auto space-y-2">
                  {mileageEntries.slice(0, 5).map(entry => <div key={entry.id} className="flex items-center justify-between text-sm p-2 bg-muted/50 rounded">
      ...
      <MileageLogDialog open={isManualDialogOpen} onOpenChange={setIsManualDialogOpen} onSave={handleManualSave} />
    </>;
}
